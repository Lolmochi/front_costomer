...
class HomePageState extends State<HomePage> {
  ...
  Future<void> _fetchCustomerData() async {
    final url =
        'http://192.168.1.109:3000/customers/${widget.customer['customer_id']}';
    try {
  ...
  Future<void> _fetchNewsImages() async {
    const url = 'http://192.168.1.109:3000/get_active_images'; // เปลี่ยน URL เป็นของ backend คุณ
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        final List newsData = json.decode(response.body);
        setState(() {
          _newsImages = List<String>.from(newsData.map((news) => 'http://192.168.1.109:3000/uploadnews/${news['image_url']}'));
        });
      } else {
            ...
            child: ClipRRect(
              ...
              child: Image.network(
                ...
                errorBuilder: (context, error, stackTrace) {
                  print('Error loading image: $error'); // ตรวจสอบข้อผิดพลาด
                  return const Center(
